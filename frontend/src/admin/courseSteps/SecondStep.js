            ...
            <div>
                ...
                {formData.videoContents.map((video, index) => (
                    <div key={index}>
                        ...
                        <button className="accordion-button rounded-4" type="button" data-bs-toggle="collapse" data-bs-target={`#collapse${index}`} aria-expanded="false" aria-controls={`collapse${index}`}>
                            Video {index+1} : {video.title}
                        </button>
                        </h2>
                        <div id={`collapse${index}`} className="accordion-collapse collapse show" aria-labelledby={`headingOne${index}`} data-bs-parent="#accordionExample">
